...
from tawq import QConv1d, QConv2d

__all__ = ['qkformer_tawq',]
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        self.fc1_conv = QConv2d(in_features, hidden_features, kernel_size=1, stride=1)
        ...
        self.fc2_conv = QConv2d(hidden_features, out_features, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        T,B,C,W,H = x.shape
        x = self.fc1_conv(x)
        x = self.fc1_bn(x.flatten(0,1)).reshape(T,B,self.c_hidden,W,H).contiguous()
        x = self.fc1_lif(x)

        x = self.fc2_conv(x)
        x = self.fc2_bn(x.flatten(0,1)).reshape(T,B,C,W,H).contiguous()
        x = self.fc2_lif(x)
        return x
...
class Token_QK_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.q_conv = QConv1d(dim, dim, kernel_size=1, stride=1)
        self.q_bn = nn.BatchNorm1d(dim)
        self.q_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')

        self.k_conv = QConv1d(dim, dim, kernel_size=1, stride=1)
        ...
        self.proj_conv = QConv1d(dim, dim, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        q_conv_out = self.q_bn(q_conv_out.flatten(0, 1)).reshape(T, B, C, N)
        q_conv_out = self.q_lif(q_conv_out)
        q = q_conv_out.unsqueeze(2).reshape(T, B, self.num_heads, C // self.num_heads, N)

        k_conv_out = self.k_conv(x)
        k_conv_out = self.k_bn(k_conv_out.flatten(0, 1)).reshape(T, B, C, N)
...
class Spiking_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.v_conv = QConv1d(dim, dim, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        q_conv_out = self.q_conv(x)
        q_conv_out = self.q_bn(q_conv_out.flatten(0, 1)).reshape(T,B,C,N).contiguous()
        q_conv_out = self.q_lif(q_conv_out)
        q = q_conv_out.transpose(-1, -2).reshape(T, B, N, self.num_heads, C//self.num_heads).permute(0, 1, 3, 2, 4).contiguous()

        k_conv_out = self.k_conv(x)
        k_conv_out = self.k_bn(k_conv_out.flatten(0, 1)).reshape(T,B,C,N).contiguous()
        k_conv_out = self.k_lif(k_conv_out)
        ...
        v_conv_out = self.v_bn(v_conv_out.flatten(0, 1)).reshape(T,B,C,N).contiguous()
        ...
        x = self.attn_lif(x)
        x = self.proj_lif(self.proj_bn(self.proj_conv(x).flatten(0,1))).reshape(T,B,C,W,H)
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj1_conv = QConv2d(embed_dims // 2, embed_dims, kernel_size=3, stride=1, padding=1)
        ...
        self.proj2_conv = QConv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1)
        self.proj2_bn = nn.BatchNorm2d(embed_dims)
        self.proj2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')

        self.proj_res_conv = QConv2d(embed_dims // 2, embed_dims, kernel_size=1, stride=2, padding=0)
    ...
    def forward(self, x):
        ...
        x_feat = x
        x = self.proj1_conv(x)
        x = self.proj1_bn(x.flatten(0, 1))
        x = self.proj1_maxpool(x).reshape(T, B, -1, H // 4, W // 4).contiguous()
        x = self.proj1_lif(x)

        x = self.proj2_conv(x)
        x = self.proj2_bn(x.flatten(0, 1)).reshape(T, B, -1, H//4, W//4).contiguous()
        x = self.proj2_lif(x)

        x_feat = self.proj_res_conv(x_feat)
        x_feat = self.proj_res_bn(x_feat.flatten(0, 1)).reshape(T, B, -1, H//4, W//4).contiguous()
...
class PatchEmbeddingStage(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj3_conv = QConv2d(embed_dims//2, embed_dims, kernel_size=3, stride=1, padding=1)
        ...
        self.proj4_conv = QConv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.proj3_conv(x)
        x = self.proj3_bn(x.flatten(0, 1))
        x = self.proj3_maxpool(x).reshape(T, B, -1, H//2, W//2).contiguous()
        x = self.proj3_lif(x)

        x = self.proj4_conv(x)
        x = self.proj4_bn(x.flatten(0, 1)).reshape(T, B, -1, H//2, W//2).contiguous()
        x = self.proj4_lif(x)

        x_feat = self.proj_res_conv(x_feat)
        x_feat = self.proj_res_bn(x_feat.flatten(0, 1)).reshape(T, B, -1, H//2, W//2).contiguous()
        x_feat = self.proj_res_lif(x_feat)
