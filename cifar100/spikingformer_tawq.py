...
class Conv3x3(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv3x3(x)
        if self.stride == 2:
            # x = self.bn(self.pool(x.flatten(0, 1))).reshape(T, B, self.dim_out, H//2, W//2)
...
class SSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.pre_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
...
class Stem(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv = AConv3x3_stem(in_channels, out_channels//8, stride=1)
    def forward(self, x):
        x = self.conv(x)
        return x
...
class Tokenizer(nn.Module):
    def __init__(self, out_channels):
        super().__init__()
        self.conv1 = Conv3x3(out_channels//8, out_channels//4, stride=1)
...
class vit_snn(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.stem(x)
        x = self.tokenizer(x)
        x = self.blocks(x).flatten(3).mean(3)
        return x
...
@register_model
def spikingformer_tawq(pretrained=False, **kwargs):
    model = vit_snn(
        **kwargs
    )
